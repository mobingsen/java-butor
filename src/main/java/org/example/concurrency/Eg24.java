 ...
 *
 * 关于线程池的总体执行策略：
 * 1.如果线程池中正在执行的线程数<corePoolSize,那么线程池就会优先选择创建新的线程执行任务而非将提交的任务加到阻塞队列中。
 ...
 * 3.当 callable对象构造完毕后，最终都会调用executor接口中声明的execute方法进行统一的处理
 *
 * 对于线程池来说，存在两个状态需要维护：
 ...
 * 3.STOP：不再接收新的任务，同时还会丢弃阻塞队列中的既有的任务；此外，它还会中断正在处理中的任务。
 ...
 * Created by mobingsen on 2020/7/14 15:00
 */
public class Eg24 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 9; i++) {
            executorService.submit(Eg24::doTask);
        }
    ...
    private static void doTask() {
        ...
        IntStream.range(0, 1).boxed().map(i -> thread.getName()).forEach(System.out::println);
    }
}
